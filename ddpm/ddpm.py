...
class LinearNoiseScheduler:
    ...
    def add_noise(self, original, noise, t):
        ...
        sqrt_one_minus_alpha_cumulative_prod = self.sqrt_one_minus_alpha_cumulative_prod[t].reshape(batch_size) # Reshaped to bx1x1x1
        
        for _ in range(len(original_shape) - 1):
            sqrt_alpha_cumulative_prod = sqrt_alpha_cumulative_prod[..., None]
            sqrt_one_minus_alpha_cumulative_prod = sqrt_one_minus_alpha_cumulative_prod[..., None]
...
def get_time_embedding(time_steps, temb_dim):
    ...
    t_emb = time_steps[:, None] / factor  # (B,1) / (temb_dim//2,) broadcasts to (B, temb_dim//2)
    ...
    def setup(self):
        self.resnet_conv_first = [
            nn.Sequential([
                ...
                nn.activation.silu,
                nn.Conv(features=self.out_channels, kernel_size=(3, 3), strides=(1, 1), padding=1),
            ])
        ...
        self.attention_norms = [nn.GroupNorm(num_groups=8) for _ in range(self.num_layers)]
        self.attentions = [nn.MultiHeadDotProductAttention(num_heads=self.num_heads) for _ in range(self.num_layers)]
        ...
        self.down_sample_conv = nn.Conv(
            features=self.out_channels, kernel_size=(4, 4), strides=(2, 2), padding=1
        ) if self.down_sample else (lambda x: x)
...
class MidBlock(nn.Module):
    ...
    def setup(self):
        ...
        self.attentions = [nn.MultiHeadDotProductAttention(num_heads=self.num_heads) for _ in range(self.num_layers)]
        self.residual_input_conv = [
            nn.Conv(features=self.out_channels, kernel_size=(1, 1)) for _ in range(self.num_layers + 1)
        ]
...
class UpBlock(nn.Module):
    ...
    def setup(self):
        ...
        self.up_sample_conv = nn.ConvTranspose(
            features=self.in_channels // 2, kernel_size=(4, 4), strides=(2, 2), padding='SAME'
        ) if self.up_sample else (lambda x: x) ## In Flax, padding=1 on a ConvTranspose pads the dilated input (not the output), which gives 14×14 instead of 16×16. The skip connection from the down path is correctly 16×16 (regular Conv padding works fine).
...
class Unet(nn.Module):
    ...
    def setup(self):
        ...
        self.conv_in = nn.Conv(features=self.down_channels[0], kernel_size=(3, 3), padding=1)
        ...
        self.conv_out = nn.Conv(features=self.img_channels, kernel_size=(3, 3), padding=1)
